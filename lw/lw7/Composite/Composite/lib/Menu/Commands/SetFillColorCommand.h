...
class SetFillColorCommand : public ICommand
{
public:
	SetFillColorCommand(ISlide& slide, std::istream& input)
		: m_slide(slide)
		, m_input(input)
	{
	}
	...
	void Execute()override
	{
		...
		m_slide.GetShapeAtIndex(position)->GetFillStyle()->SetColor(
			{ redColor, greenColor, blueColor, alpha }
		);
		m_slide.GetShapeAtIndex(position)->GetFillStyle()->Enable(true);
	}
