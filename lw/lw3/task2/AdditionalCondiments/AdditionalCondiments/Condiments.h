...
{
public:
	CChocolate(IBeveragePtr&& beverage, unsigned int sliceCount)
		: CCondimentDecorator(move(beverage))
	{
		if (sliceCount > m_MAXIMUM_SLICES_COUNT)
		{
			std::invalid_argument(
				"Maximum amount of slices is " + std::to_string(m_MAXIMUM_SLICES_COUNT)
			);
		}
...
protected:
	double GetCondimentCost()const override
	{
		return 25 * m_sliceCount;
	}

	std::string GetCondimentDescription()const override
	{
		return "Chocolate slices " + std::to_string(m_sliceCount);
	}

private:
	const unsigned int m_MAXIMUM_SLICES_COUNT = 5;
	unsigned int m_sliceCount;
};

enum class CLiquorType
{
	Nutty = 0,
	Chocolate,
};
...
{
public:
	CLiquor(IBeveragePtr&& beverage, CLiquorType type)
		: CCondimentDecorator(move(beverage))
		, m_type(type)
	{}
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		return std::string(m_type == CLiquorType::Chocolate ? "Chocolate" : "Nutty") + " liquor";
	}
